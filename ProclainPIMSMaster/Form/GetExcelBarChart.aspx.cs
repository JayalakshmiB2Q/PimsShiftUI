    ...
    public partial class GetExcelBarChart : System.Web.UI.Page
    {
        ...
        private void labelview()
        {
            try
            {
                ...
                else if (data.ToString() == "Supervisor")
                {
                    SuperExcel(data);

                }
                else if (data.ToString() == "ManNew")
                {
                    SuperExcel(data);
        ...
        protected void ManNewExcel(string data)
        {
            DataSet Ds = new DataSet();
            Employee E = new Employee();
            try
            {
                ...
                using (XLWorkbook wb = new XLWorkbook()) //dataset to excel but it need closedXML.dll
                {
                    ...
                    int count = dt.Columns.Count;
                    int cc = count / 2;
                    ws.Cell(1, cc).Value = "PIMS Manager NEW SUGGESTIONS";
                    ws.Cell(1, cc).Style.Font.Bold = true;
                    ws.Cell(1, cc).Style.Font.FontSize = 25;
                    ws.Cell(1, cc).Style.Font.FontName = "Calibri";
                    Response.Clear();
        ...
        protected void SuperExcel(string data)
        {
            DataSet Ds = new DataSet();
            Employee E = new Employee();
            try
            {
                ...
                using (XLWorkbook wb = new XLWorkbook()) //dataset to excel but it need closedXML.dll
                {
                    ...
                    int count = dt.Columns.Count;
                    int cc = count / 2;
                    ws.Cell(1, cc).Value = "PIMS Supervisor Report";
                    ws.Cell(1, cc).Style.Font.Bold = true;
                    ws.Cell(1, cc).Style.Font.FontSize = 25;
                    ws.Cell(1, cc).Style.Font.FontName = "Calibri";
                    Response.Clear();
