        ...
        protected void ManagerButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (z == 1)
                {
                    Clear();
                    //SuggestionTableView();
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "swal('P A Y M E N T','Paid Successfully','success').then(function() {window.location = 'FinanceDashboard.aspx'})", true);
                }
            }
            catch (Exception e1)
            {
