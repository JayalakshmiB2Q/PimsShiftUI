...
namespace ProclainPIMSMaster.Form
{
    ...
    public string iremark { get; set; }
        ...
        public string IEmpId1 { get; set; }
        public string IEmpName1 { get; set; }
        public string IDepID1 { get; set; }
        public string SUP_ID { get; set; }
        public string SUP_Name { get; set; }
        public string SUP_MAIL { get; set; }
        public string SUP_Dept { get; set; }
        public string SUP_DAte { get; set; }
        public string SUP_Remark { get; set; }
        public string SU_ID { get; set; }
        public string SU_Name { get; set; }
        public string SU_MAIL { get; set; }
        public string SU_Dept { get; set; }
        public string SU_DAte { get; set; }
        public string SU_Remark { get; set; }
        public string impid { get; set; }
    public string implname { get; set; }
    public string impdept { get; set; }
    public string impmail { get; set; }
        ...
        public void Upinsert()
         {
            try
            {
                if (index != "" && index != "System.Data.DataRowView")
                {
                    DBM.ComInboxID = index.ToString();
                   // string check;
                  //  check = RepCheckRadioButtonList.SelectedItem.ToString();
                    DBM.Status = RepCheckRadioButtonList.SelectedItem.Text.ToString();
                  if (RepCheckRadioButtonList.SelectedItem.Text.ToString() == null || RepCheckRadioButtonList.SelectedItem.Text.ToString() == "")
                    {
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "swal('OOPS','Please select any one Accepted or Rejected','warning')", true);
                    }
                    if (RepCheckRadioButtonList.SelectedItem.Text.ToString() == "Accepted")
                    {
                        DBM.Reason = "NULL";
                    }
                    else
                    ...
                    //string myscript = "alert ('Thanks For Response');";
                    //Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "MyScript", myscript, true);
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "swal('W O W','Thanks For Your Response','success').then(function() {window.location = 'CommitteeEvalDashboard.aspx'})", true);
                }
                else
                {
                    //string myscript = "alert ('Forget To Select the IDea ID');";
                    //Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "MyScript", myscript, true);
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "swal('Forget To Select the IDea ID','','warning')", true);

                }
            }
            catch (Exception e1)
            {
                //alert ('Forget To Select the IDea ID');";
