...
namespace ProclainPIMSMaster.Form
{
    ...
    public partial class FinanceImplDashboard : System.Web.UI.Page
    {
        ...
        public void FinanceEvaluvationInsert()
        {
            ...
            try
            {
                MM.IdeaId = IMPIdeaIdTextBox.Text.ToString().TrimStart().TrimEnd();
              ...
              //  ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "swal('Paid For Implement Members','','success')", true);
        ...
        protected void ManagerButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (z == 1)
                {
                    Clear();
                    //SuggestionTableView();
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "swal('P A Y M E N T','Paid Successfully','success').then(function() {window.location = 'FinanceImplDashboard.aspx'})", true);
                }
            }
            catch (Exception e1)
            {
        ...
        {
            string IdeaID = IMPIdeaIdTextBox.Text.TrimStart().TrimEnd().ToString();
            string cmd = SubjectIMPTextBox.Text.ToString();
                        Response.Redirect("FinanceExcel.aspx?IdeaID="+ IdeaID + "&cmd="+ cmd); 
        }
    }
