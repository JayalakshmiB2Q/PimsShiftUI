...
namespace ProclainPIMSMaster.Form
{
    ...
    public partial class CommiteeEvalSend : System.Web.UI.Page
    {
        ...
        protected void ManagerButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (z == 1)
                {
                    Clear();
                    SuggestionTableView();
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "swal('SUGGESSTION','Send To Manager','success').then(function() {window.location = 'CommiteeEvalSend.aspx'})", true);
        ...
        protected void Alterreject_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string value = RepCheckRadioButtonList.SelectedItem.Value.ToString();
                if (value == "Rejected")
                {
                    
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "swal('suggesstion rejected','','info')", true);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "swal('suggesstion Accepted','','info')", true);
                }
